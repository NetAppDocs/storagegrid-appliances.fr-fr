...
NOTE: Les modifications apportées à la configuration IP lorsque l'appliance est en mode de maintenance ne sont pas appliquées à l'environnement StorageGRID installé. Exécutez le lien :../maintain/changing-nodes-network-configuration.html[`change-ip` Commande] après le redémarrage de l'appliance dans StorageGRID.
