...
---
= Remplacez les disques durs d'un SGF6112
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
. Si plusieurs disques ont subi une panne et ont été remplacés, des alertes peuvent vous indiquer que certains volumes doivent être restaurés. Si vous recevez une alerte, avant de tenter une récupération de volume, sélectionnez *Nœuds* > `*appliance Storage Node*` > *Matériel*. Dans la section Appliance StorageGRID de la page, vérifiez que le mode RAID de stockage est sain ou en cours de reconstruction. Si l’état indique un ou plusieurs disques défaillants, corrigez ce problème avant de tenter une restauration de volume.
