...
---
= Exigences pour le clonage des nœuds de l'appliance StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
+
Le nœud source et l'appliance cible peuvent avoir des paramètres de chiffrement de nœud différents, à quelques exceptions près. Le clonage de nœuds n'est pas pris en charge depuis des sources non chiffrées vers des cibles chiffrées pour les appliances suivantes :
...
** Passer d'un dispositif SG1200 sans chiffrement à un dispositif SG1200 avec chiffrement
** Un appareil de nœud SG110 sans cryptage de nœud vers un appareil SG110 avec cryptage de nœud
** Passer d'un dispositif de nœud SG120 sans chiffrement à un dispositif SG120 avec chiffrement
...
* Déterminez si le mode RAID de l'appliance cible doit être modifié par rapport à sa configuration par défaut afin de pouvoir spécifier cette information lors de l'installation initiale de l'appliance cible avant le clonage. Vous pouvez consulter les informations relatives au mode RAID actuel du nœud de l'appliance source sur la page *Nodes* de Grid Manager. Sélectionnez l'onglet *Hardware* de l'appliance.
+
