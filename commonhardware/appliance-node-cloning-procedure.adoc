...
---
= Cloner un nœud d'appliance StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
+
En cas de problèmes empêchant le clonage, *Démarrer le clonage* n'est pas activé et les problèmes que vous devez résoudre sont affichés comme *État de la connexion*. Ces problèmes sont listés sur la page d'accueil de l'installateur StorageGRID Appliance, aussi bien sur le nœud source que sur l'appliance cible. Un seul problème est affiché à la fois et l'état est mis à jour automatiquement à mesure que les conditions changent. Résolvez tous les problèmes de clonage pour activer *Démarrer le clonage*.
