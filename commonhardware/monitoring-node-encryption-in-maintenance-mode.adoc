...
---
= Surveiller le chiffrement d'un appareil StorageGRID en mode maintenance
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
* Après la mise hors service d'un appareil sur lequel le chiffrement des nœuds était activé.
...
+
La clé de chiffrement KMS et toutes les données sont supprimées du nœud et l'appliance redémarre.  Une boîte de dialogue s'affiche pendant le redémarrage de l'appareil, ce qui peut prendre jusqu'à 20 minutes.
