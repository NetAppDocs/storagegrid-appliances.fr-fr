...
---
= Comment fonctionne le clonage de nœuds d'appliance StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
** Le nombre de volumes du périphérique de stockage cible doit être supérieur ou égal au nombre de volumes du nœud source. Le SGF6112 possède 12 volumes et le SGF6212 possède 13 volumes. Tous les autres périphériques de stockage possèdent 16 volumes.
...
+
Vous pouvez consulter les informations relatives au mode RAID actuel du nœud source sur la page *Nodes* de Grid Manager. Sélectionnez l'onglet *Storage* pour l'appliance.

*** Vous ne pouvez pas cloner un nœud source avec 16 volumes de stockage d'objets (rangedb) vers un dispositif de stockage cible avec moins de volumes de stockage d'objets, même si le dispositif cible a une capacité supérieure à celle du nœud source.
...
=== Informations et restrictions relatives au clonage de l'appareil SG120

Vous pouvez cloner un nœud source SG120 sur un nœud de services cible SG1200 afin d'offrir au nœud d'administration ou au nœud de passerelle des capacités accrues.

Le clonage de nœuds n'est pas pris en charge à partir d'un nœud source SG120 sans chiffrement vers un nœud cible SG120 avec chiffrement.
...
=== Informations et restrictions relatives au clonage de l'appareil SG1200

Vous pouvez cloner un nœud source SG1200 sur une appliance cible de services SG120.

* Le clonage du SG1200 sur un dispositif cible de services SG1200 permet de redéployer le SG1200 pour une application plus exigeante. Par exemple, si un dispositif SG1200 source est utilisé comme nœud d'administration et que vous souhaitez l'utiliser comme nœud dédié à l'équilibrage de charge.
* Le remplacement d'un nœud source SG1200 par un nœud cible de services SG1200 réduit la vitesse maximale des ports réseau de 100-GbE à 25-GbE.


Le clonage de nœuds n'est pas pris en charge à partir d'un nœud source SG1200 sans chiffrement vers un nœud cible SG1200 avec chiffrement.
...
* Un dispositif de nœud source SG5760 configuré avec DDP ou DDP-16 _ne peut pas_ être cloné sur un dispositif cible de stockage SG6160 ou SG6260 avec un mode RAID correspondant.
...
* Vous pouvez cloner un nœud source SG6060 sans aucun châssis d'extension vers un SG6160 ou SG6260 appliance de stockage cible sans aucun châssis d'extension si les tailles de disque et les modes RAID sont identiques.
* Un dispositif de nœud source SG6060 avec un nombre quelconque d'étagères d'extension _ne peut pas_ être cloné sur un dispositif cible de stockage SG6160 ou SG6260 même si le nombre d'étagères d'extension, les tailles de disques et les modes RAID sont les mêmes.
...
Le clonage de nœuds n'est pas pris en charge à partir d'un nœud source SGF6112 sans chiffrement vers un nœud cible SGF6112 ou SGF6212 avec chiffrement.



=== Informations et restrictions relatives au clonage des appliances SG6160 et SG6260
