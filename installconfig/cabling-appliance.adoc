...
---
= Câbler un appareil StorageGRID ou un contrôleur
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
+
** Un à quatre câbles TwinAx pour la connexion des quatre ports réseau.
** Pour le SG120, un à quatre émetteurs-récepteurs SFP+ ou SFP28 si vous prévoyez d'utiliser des câbles optiques pour les ports.
** Pour le SG1200, un à quatre émetteurs-récepteurs QSFP+, QSFP28 ou QSFP56 (200GbE NIC uniquement) si vous prévoyez d'utiliser des câbles optiques pour les ports.
...
Connexions du port SG120 :

image::../media/sg120_connections.png[Connecteurs arrière SG120]

Connexions du port SG1200 :

image::../media/sg1200_connections.png[connexions de port SG1200]
...
* 10/25/40/100-GbE avec carte réseau 100G
* 10/25/50/100/200-GbE (200GbE NIC uniquement)

 a| 
Connectez-vous au réseau Grid et au réseau client pour StorageGRID. Voir link:gathering-installation-information-sg120-and-sg1200.html#port-bond-modes["Modes de liaison des ports (SG120 et SG1200)"]
...
[cols="2a,2a"]
|===
| Vitesse de liaison SG120 (GbE) | Équipement requis 
...
[cols="2a,2a"]
|===
| Vitesse de liaison SG1200 (GbE) | Équipement requis 
...
Émetteur-récepteur QSFP28
 ...
 a| 
200 (carte réseau 200GbE uniquement)
 a| 
Émetteur-récepteur QSFP56
...
.Avant de commencer
* SG6200 uniquement : Vous disposez du câble de dérivation 100 GbE vers 4x25 GbE fourni avec l’appareil pour connecter les deux contrôleurs de stockage au contrôleur SG6200-CN.
* Vous disposez de câbles Ethernet RJ-45 :
+
** Un câble RJ-45 pour connecter le port de gestion.
** SG6200 uniquement : jusqu’à quatre câbles Ethernet RJ-45 supplémentaires pour les ports optionnels que vous prévoyez d’utiliser, y compris le deuxième port d’administration et le port BMC sur le SG6200-CN, ainsi que les ports de maintenance sur chacun des deux contrôleurs E4000.


* Vous avez l'une des options suivantes pour les ports réseau. Ces éléments ne sont pas fournis avec l'appareil.
+
** Un à quatre câbles TwinAx pour la connexion des quatre ports réseau.
** Un à huit émetteurs-récepteurs SFP+, SFP28, QSFP+, QSFP28 ou QSFP56 si vous prévoyez d'utiliser des câbles optiques pour les ports.
...
.Description de la tâche
Les figures suivantes illustrent les ports situés à l'arrière du SGF6212 et des trois contrôleurs de l'appliance SG6200. L'appliance SG6200 comprend un contrôleur de calcul SG6200-CN en haut et deux contrôleurs de stockage E4000 en bas.

Connexions SGF6212 :

image::../media/sgf6212_connections.png[Connecteurs arrière SGF6212]

Connexions SG6260 :

image::../media/sg6200_e4000_connections.png[Connexions SG6200-CN à E4000]
...
2
 a| 
Quatre ports réseau sur le contrôleur SG6200-CN
 a| 
* Quatre ports réseau 10/25/40/100-GbE ou 200-GbE avec option 200G NIC SKU

 a| 
Connectez-vous au réseau Grid et au réseau client pour StorageGRID. Voir link:gathering-installation-information-sg6200.html#port-bond-modes["Modes de liaison des ports (SG6200)"]
...
Cinq ports de connexion au total
 a| 
* Un port 100GbE sur le SG6200-CN
* Deux ports 10 GbE sur chacun des contrôleurs de stockage

 a| 
Connectez chaque contrôleur de stockage au contrôleur SG6200-CN.
...
. Connectez les deux ports d'interconnexion de chaque contrôleur de stockage au port 100GbE du contrôleur SG6200-CN, à l'aide d'un câble QSFP28 100-GbE vers 4x25-GbE SFP28.
. Connectez les ports réseau de l'appareil aux commutateurs réseau appropriés à l'aide de câbles TwinAx ou de câbles optiques et d'émetteurs-récepteurs.
+
[cols="2a,2a"]
|===
...
. Si vous envisagez d'utiliser le réseau d'administration pour StorageGRID, connectez le port réseau d'administration de l'appliance au réseau d'administration à l'aide d'un câble Ethernet.
. Si votre appareil comprend un tiroir d'extension, consultez le link:../sg6200/adding-expansion-shelf-to-deployed-sg6260.html["Instructions pour ajouter un tiroir d'extension à un SG6200 déployé"] pour les informations relatives au câblage.
