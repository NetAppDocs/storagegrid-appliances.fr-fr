...
---
= Configurer SANtricity pour les appliances StorageGRID SG6260, SG6160, SG6000, SG5700 et SG5800
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
Les appliances StorageGRID suivantes peuvent accéder à SANtricity System Manager :
...
* Clés de sécurité du lecteur, nécessaires pour déverrouiller les lecteurs sécurisés
+

NOTE: Vous devez configurer SANtricity System Manager et y avoir accès si la fonction Drive Security est activée pour votre appareil.
...
** Utilisez le Grid Manager et sélectionnez *Nœuds* > `*_appliance Storage Node_*` > *SANtricity System Manager*
...
.SG6260
--
Le système de stockage SG6260 peut être équipé de disques conformes à la norme FIPS, aussi bien dans le contrôleur de calcul SG6200-CN que dans le châssis de contrôleur E4000. Le chiffrement des disques est configuré séparément pour les disques SG6200-CN et les disques E4000.

.. link:optional-enabling-node-encryption.html#drive-encryption["Activez Drive Encryption"] pour les SSD SED installés dans le nœud de calcul SG6200-CN.
.. Créez une clé de sécurité (une chaîne de caractères partagée par le contrôleur et des lecteurs pour l'accès en lecture/écriture).
+
*** Vous pouvez https://docs.netapp.com/us-en/e-series-santricity/sm-settings/create-internal-security-key.html["créer une clé interne"^] utiliser la mémoire persistante du contrôleur ou une clé externe fournie par un serveur de gestion des clés.
*** Pour utiliser une clé externe fournie par un serveur de gestion des clés, vous devez d'abord utiliser https://docs.netapp.com/us-en/e-series-santricity/sm-settings/overview-drive-security.html["établissez l'authentification avec un serveur de gestion des clés"^] SANtricity System Manager.
...
NOTE: Suivez cette procédure uniquement si vous n'avez pas accès à SANtricity System Manager depuis l'appliance StorageGRID Installer (*Advanced* > *SANtricity System Manager*) ou Grid Manager (*Nodes* > *SANtricity System Manager*).
