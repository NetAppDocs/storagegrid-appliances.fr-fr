...
---
= Configurer les adresses IP de l'appliance StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Utilisez le programme d'installation de l'appliance StorageGRID pour configurer les adresses IP et le routage de l'appliance de services ou du nœud de stockage sur les réseaux Grid, Admin et Client.
...
Vous devez soit attribuer une adresse IP statique à l'appareil sur chaque réseau réseau ou réseau d'administration connecté, soit attribuer un bail permanent pour l'adresse sur le serveur DHCP.  L'adresse IP statique ou la configuration DHCP est facultative pour un réseau client connecté.

Pour activer ou désactiver un lien ou modifier la configuration du lien, consultez les instructions suivantes :
...
* link:../sg120-1200/changing-link-configuration-of-sg120-and-sg1200-appliance.html["Modifier la configuration de liaison de l'appliance de services SG120 ou SG1200"]
...
* link:../sg6200/changing-link-configuration-of-sg6200-appliance.html["Modifier la configuration de liaison de l'appliance SG6200"]
...
. Pour configurer le réseau en grille, sélectionnez *Statique* ou *DHCP* dans la section *Réseau en grille* de la page, puis entrez vos paramètres réseau.
+
[role="tabbed-block"]
====
.Statique
...
NOTE: Si le réseau client n'est pas activé, l'itinéraire par défaut utilisera la passerelle du réseau Grid.
...
--
====
. Pour configurer le réseau d'administration, sélectionnez *Statique* ou *DHCP* dans la section *Réseau d'administration* de la page, puis entrez vos paramètres réseau.
+
...
+
Voirlink:../installconfig/cabling-appliance.html["Appareil câblé"] pour l'emplacement du port de gestion 1 sur votre appareil.
...
--
====
. Pour configurer le réseau client, sélectionnez *Statique*, *DHCP* ou *Aucun* dans la section *Réseau client* de la page, puis entrez vos paramètres réseau.
+

NOTE: Pour configurer le réseau client, assurez-vous que le réseau client est activé sur la page Configuration du lien.
...
.Aucune
--
Sélectionnez *Aucun* pour activer le réseau client sans spécifier d’adresse IP.  Le réseau client n'a besoin que d'une adresse IP pour un accès direct.  L'activation du réseau client sans adresse IP vous permet de configurer les interfaces VLAN du réseau client dans StorageGRID.
