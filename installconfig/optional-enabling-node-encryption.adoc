...
---
= Activez le chiffrement des nœuds ou des disques pour les appliances StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
De plus, lorsque vous activez le mode FIPS après avoir activé le chiffrement des nœuds, le module NetApp StorageGRID Kernel Crypto API 6.1.129-1-ntap1-amd64 est utilisé pour le chiffrement des données au repos. Consultez  https://docs.netapp.com/us-en/storagegrid/admin/manage-tls-ssh-policy.html#select-a-security-policy["Sélectionnez une politique de sécurité"^] pour plus d'informations.
...
Le chiffrement des disques peut être utilisé pour tout SSD SED installé dans un nœud de calcul ou un contrôleur SG100, SG1000, SG110, SG1100, SG120, SG1200, SGF6112, SGF6212, SG6100-CN ou SG6200-CN.

* Pour les appliances de services, les disques SSD correspondent aux disques racines des nœuds.
* Dans un contrôleur SG6100-CN ou SG6200-CN, les SSD sont utilisés pour la mise en cache.
* Dans un SGF6112 ou SGF6212, les SSD sont les disques racine du nœud et sont utilisés pour le stockage principal des données d'objet.
