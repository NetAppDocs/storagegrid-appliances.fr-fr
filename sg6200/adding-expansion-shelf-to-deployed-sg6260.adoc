...
summary: 'Pour augmenter la capacité de stockage, vous pouvez ajouter un ou deux tiroirs d"extension à un SG6260 déployé dans un système StorageGRID.' 
---
= Ajouter un tiroir d'extension au SG6260 déployé
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Pour augmenter la capacité de stockage, vous pouvez ajouter un ou deux tiroirs d'extension à un SG6260 déjà déployé dans un système StorageGRID.

.Description de la tâche
Pour ajouter un tiroir d'extension, procédez comme suit :

* Installez le matériel dans l'armoire ou le rack.
* Mettez le SG6260 en mode maintenance.
...
L'installation d'une ou deux baies d'extension devrait prendre une heure maximum par nœud d'appliance. Pour minimiser l'interruption, les étapes suivantes vous indiquent d'installer les nouvelles baies d'extension et les disques avant de mettre le SG6260 en mode maintenance. Les étapes restantes devraient prendre environ 20 à 30 minutes par nœud d'appliance.

.Avant de commencer
* Vous devez disposer de la phrase secrète pour le provisionnement.
* Vous devez utiliser StorageGRID 12.0 ou une version ultérieure.
* Le tiroir d'extension et deux câbles SAS pour chaque tiroir d'extension sont disponibles.
* Vous avez link:locating-sg6200-in-data-center.html["situé physiquement le contrôleur"] où vous ajoutez le tiroir d'extension dans le centre de données.


.Étapes
. Suivez les instructions pour link:../installconfig/sg6260-installing-60-drive-shelves-into-cabinet-or-rack.html["installation des tiroirs de 60 disques dans une armoire ou un rack"].
. Depuis le gestionnaire de grille, link:../commonhardware/placing-appliance-into-maintenance-mode.html["mettre le contrôleur SG6200-CN en mode maintenance"].
...
+
image::../media/expansion_shelves_connections_sg6260.png[Connexions du tiroir d'extension SG6260]
...
+
Exemples de messages pouvant apparaître sur la page d'accueil, en fonction du nombre de tiroirs d'extension existants ou nouveaux :
...
+
Une fois le redémarrage terminé, l'onglet *Tâches* s'affiche avec des options permettant de redémarrer le nœud ou de placer l'appliance en mode maintenance.

. Vérifiez l'état du nœud de stockage de l'appliance et des nouveaux tiroirs d'extension.
+
.. Dans le Gestionnaire de grille, sélectionnez *Nœuds* et vérifiez que le nœud de stockage de l'appliance possède une icône de coche verte.
+
